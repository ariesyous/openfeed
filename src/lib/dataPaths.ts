import { topicSlug } from "../../shared/topics";
 ...
 */
export function manifestUrl(topic = ""): string {
  const bucketMs = 5 * 60_000;
  const bucket = Math.floor(Date.now() / bucketMs);
  // Vite development serves source data; production emits per-topic indexes.
  const file = topic && import.meta.env.PROD ? `topics/${topicSlug(topic)}/manifest.json` : "manifest.json";
  return `${dataUrl(file)}?t=${bucket}`;
}
