...
import { BatchFileSchema, ManifestSchema, type FeedItem, type Manifest } from "../schemas";
...
export function articleMarkup(item: FeedItem, base: string): string {
  ...
  return `<div class="app"><header class="app-header"><a class="app-title" href="${base}">OpenFeed</a></header><main class="app-main"><article class="post-card"><p>${escape(editorial.format)} · AI-edited · ${escape(item.createdAt.slice(0, 10))}</p><h1>${escape(item.title ?? "Article")}</h1>${editorial.spoilers ? `<details><summary>Show spoilers</summary>${reading}</details>` : reading}<section class="source-panel"><h2>Based on publisher excerpts</h2><ul>${sources}</ul></section></article><a href="${base}">← Back to feed</a></main></div>`;
}
...
export function buildArchive(root = process.cwd()) {
  ...
  writeFileSync(path.join(dist, "404.html"), `<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>Article not found · OpenFeed</title><body><h1>Article not found</h1><p>This address doesn't match a published article.</p><a href="${base}">Return to OpenFeed</a></body></html>`);
  ...
  console.log(`[archive] ${count} permanent articles; ${(total / 1024 / 1024).toFixed(2)} MiB deployed output; ${INDEX_PAGE_SIZE} batch refs per index page.`);
