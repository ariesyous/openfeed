...
describe("reviewed editorial integrity examples", () => {
  it("preserves supported attribution, uncertainty, commentary and corrected factual examples", () => {
    ...
    expect(INTEGRITY_EXAMPLES).toHaveLength(24);
    expect(new Set(results.map(result => result.id)).size).toBe(24);
  ...
  it("reports semantic judgments separately from executable publication gates", () => {
    const results = reviewIntegrityBenchmark();
    for (const id of ["comet-entity-wrong", "crater-scope-wrong", "family-relationship-wrong", "discussion-background-wrong", "live-unrelated-source-wrong", "live-speaker-attribution-wrong", "manual-growth-periods-wrong", "manual-title-denominator-wrong", "manual-nonrival-price-wrong", "manual-asteroid-class-wrong"]) {
